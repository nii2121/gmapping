...
int GridSlamProcessorThread::init(int argc, const char * const * argv){
  //cout << "here now!8" << endl;
	m_argc=argc;
	m_argv=argv;
	std::string configfilename;
	std::string ebuf="not_set";
	//cout << "here now!6" << endl;
...
// 	  critscore = cfg.value("gfs","critscore",critscore);
	  ...
	  autoParams = (bool) cfg.value("gfs","automatic_parameter_estimation", autoParams);
// 	  maxMove = cfg.value("gfs","maxMove",maxMove);
	  ...
	  m_covScanmatch = cfg.value("gfs","covScanmatch", m_covScanmatch);
...
// 		parseDouble("-maxMove",maxMove);
		...
		parseDouble("-autoparams",autoParams);
		if (autoParams) {
		  parseDoubleSilent("-sigma",sigma);
		  parseDoubleSilent("-lstep",lstep);
		  parseDoubleSilent("-lsigma", lsigma);
		} 
		else {
		...
		parseFlag("-covScanmatch", m_covScanmatch);
	...
	if (autoParams)
	  estimateResolutionDependendParameters(delta);

	//if ((filename.length() <=0)&&(onLine!=0) ){
	//cout << "here now!1" << endl;
	if ((filename.length() <=0)&&(onLine!=1)){
	  //cout << "here now!" << endl;
	  cout << "no filename specified " << onLine << "!" << endl;
	  return -1;
	}
...
void * GridSlamProcessorThread::fastslamthread(GridSlamProcessorThread* gpt){
	...
	gpt->setperformScanmatching(!gpt->skipMatching);
	gpt->GridSlamProcessor::init(gpt->particles, xmin, ymin, xmax, ymax, gpt->delta, initialPose, gpt->m_covScanmatch);
	...
	gpt->setlinearOdometryReliability(gpt->linearOdometryReliability);
	gpt->setangularOdometryReliability(gpt->angularOdometryReliability);
	gpt->setconsiderOdometryCovariance(gpt->considerOdometryCovariance);
...
void GridSlamProcessorThread::estimateResolutionDependendParameters(double res) {
  delta = res;
  lsigma=delta;
  lstep=delta;
  sigma=0.5*delta;
